...
void bin2ord(btree<int> &B, tree<int> &T, btree<int>::iterator itB,tree<int>::iterator itT){
	...
	*itT=*itB;
	...
	if(itB.left() != B.end() || itB.right()!=B.end()){
		///Inserta preventimvamente dos valores LAMBDA en los hijos de t
		l = itT.lchild();
		l = T.insert(l, -1);
		...
		///Recursivamnte la funcion se encarga de setear el valor si el correspondiente nodo de B no es end
